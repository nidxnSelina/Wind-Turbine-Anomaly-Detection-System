...
def load_xls(val_data_path: str, required_english_cols: list) -> pd.DataFrame:
    """
    Load and standardize multiple Excel (.xls/.xlsx) files containing turbine times-series data.

    This function:
      1. Reads each Excel file into a DataFrame.
      2. Renames columns into English using a predefined mapping.
      3. Ensures that all required columns exist, adding empty ones if missing.
      4. Concatenates all sheets into one combined DataFrame.

    Args:
        val_data_path (str): Path to a folder containing .xls or .xlsx files.
        required_english_cols (list): List of required column names

    Returns:
        pd.DataFrame: Combined and cleaned dataset containing only the required columns.
            Missing columns are filled with NA values.
    """
    ...
    for file in os.listdir(val_data_path):
        ...
        # Normalize odd punctuation/spaces
        df.columns = (
    ...
    # Combine all datasets and fill NA
    data = pd.concat(frames, ignore_index=True)
    for c in required_english_cols:
        if c not in data.columns:
            data[c] = pd.NA

    # Reorder to required list and sanitize column names
